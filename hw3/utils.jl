...
function showmesh(X::Matrix{Float64}, T::Matrix{Int};
                  frame=false)
    scene = mesh(X, T, color=X[:,2], shading=true)
    if frame
...
"""
function updatemesh!(scene, X::Matrix{Float64}, T::Matrix{Int})
    scene = mesh(X, T, color=X[:,2], shading=true)
end
...
function showdescriptor(X::Matrix{Float64},
    ...
    scene = mesh(X, T, color=desc, shading=false)
    if frame
        wireframe!(scene[end][1], color=(:black, 0.6), linewidth=2)
    end
    scene
