...
### Problem 3
In this problem, you will implement a special case of the "Discrete Elastic Rods" paper - a closed loop formed from naturally straight, isotropic rod.

**a)** Add code to compute the $3 \times 3 \times n$ array `curveData.parallelTransport`. Each $3 \times 3$ page of this array is a rotation matrix aligning the Bishop frames between successive edges (denoted $P_i$ in the paper).

**answer:** See `elasticRods.jl`.

**b)** The bending energy has the form
```math
E_{bend} = \alpha \sum_i \frac{|(\kappa\mathbf{b})_i|^2}{l_i},
```
where $l_i$ is the dual length of vertex $i$. The bending force at vertex $i$ is thus
```math
-\nabla_i E_{bend} = \sum_j \frac{-2α}{l_j} (\nabla_i(\kappa\mathbf{b})_j)^\top (\kappa\mathbf{b})_j.
```
Show that
```math
\nabla_i(\kappa\mathbf{b})_j =
\frac{1}{|\mathbf{e}^{j-1}| |\mathbf{e}^{j}| + \mathbf{e}^{j-1} \cdot \mathbf{e}^{j}}
\begin{cases}
   2[\mathbf{e}^{j}]   + (\kappa\mathbf{b})_j (\mathbf{e}^{j})^\top   & i=j−1 \\
   2[\mathbf{e}^{j-1}] − (\kappa\mathbf{b})_j (\mathbf{e}^{j-1})^\top & i=j+1 \\
  -2[\mathbf{e}^{j}]   - (\kappa\mathbf{b})_j (\mathbf{e}^{j})^\top
  -2[\mathbf{e}^{j-1}] + (\kappa\mathbf{b})_j (\mathbf{e}^{j-1})^\top & i=j \\
   0 & \text{otherwise},
\end{cases}
```
where $[\mathbf{x}]$ denotes a $3 \times 3$ matrix defined so that $[\mathbf{x}]\mathbf{y} = \mathbf{x} \times \mathbf{y}$.

**answer:** First, let us write down derivatives of dot and cross product of two adjacent edges
```math
\nabla_i(\mathbf{e}^{j-1} \cdot \mathbf{e}^{j}) =
\begin{cases}
  -(\mathbf{e}^{j})^\top   & i=j−1 \\
   (\mathbf{e}^{j})^\top
  -(\mathbf{e}^{j-1})^\top & i=j \\
   (\mathbf{e}^{j-1})^\top & i=j+1 \\
   0 & \text{otherwise}
\end{cases}
```
```math
\nabla_i(\mathbf{e}^{j-1} \times \mathbf{e}^{j}) =
\begin{cases}
   [\mathbf{e}^{j}]   & i=j−1 \\
  -[\mathbf{e}^{j}]
  -[\mathbf{e}^{j-1}] & i=j \\
...
\end{cases}
```
Having $(\kappa\mathbf{b})_j = 2 (\mathbf{e}^{j-1} \times \mathbf{e}^{j}) / (|\mathbf{e}^{j-1}| |\mathbf{e}^{j}| + \mathbf{e}^{j-1} \cdot \mathbf{e}^{j})$, to compute its derivatives, we can start with quotient rule. Also, using the inextensibility property of the rod, we conclude $\nabla_i (|\mathbf{e}^{j-1}| |\mathbf{e}^{j}| + \mathbf{e}^{j-1} \cdot \mathbf{e}^{j}) = \nabla_i (\mathbf{e}^{j-1} \cdot \mathbf{e}^{j})$. Thus:
```math
\begin{align*}
  \nabla_i (\kappa\mathbf{b})_j
    &= \frac{2\nabla_i(\mathbf{e}^{j-1} \times \mathbf{e}^{j}) (|\mathbf{e}^{j-1}| |\mathbf{e}^{j}| + \mathbf{e}^{j-1} \cdot \mathbf{e}^{j}) - 2(\mathbf{e}^{j-1} \times \mathbf{e}^{j}) \nabla_i(|\mathbf{e}^{j-1}| |\mathbf{e}^{j}| + \mathbf{e}^{j-1} \cdot \mathbf{e}^{j})}{(|\mathbf{e}^{j-1}| |\mathbf{e}^{j}| + \mathbf{e}^{j-1} \cdot \mathbf{e}^{j})^2} \\
    &= \frac{2\nabla_i(\mathbf{e}^{j-1} \times \mathbf{e}^{j})}{|\mathbf{e}^{j-1}| |\mathbf{e}^{j}| + \mathbf{e}^{j-1} \cdot \mathbf{e}^{j}} -
       \frac{2\mathbf{e}^{j-1} \times \mathbf{e}^{j}}{|\mathbf{e}^{j-1}| |\mathbf{e}^{j}| + \mathbf{e}^{j-1} \cdot \mathbf{e}^{j}} \cdot
       \frac{\nabla_i(|\mathbf{e}^{j-1}| |\mathbf{e}^{j}| + \mathbf{e}^{j-1} \cdot \mathbf{e}^{j})}{|\mathbf{e}^{j-1}| |\mathbf{e}^{j}| + \mathbf{e}^{j-1} \cdot \mathbf{e}^{j}} \\
    &= \frac{2\nabla_i(\mathbf{e}^{j-1} \times \mathbf{e}^{j})}{|\mathbf{e}^{j-1}| |\mathbf{e}^{j}| + \mathbf{e}^{j-1} \cdot \mathbf{e}^{j}} -
       (\kappa\mathbf{b})_j \cdot
       \frac{\nabla_i(\mathbf{e}^{j-1} \cdot \mathbf{e}^{j})}{|\mathbf{e}^{j-1}| |\mathbf{e}^{j}| + \mathbf{e}^{j-1} \cdot \mathbf{e}^{j}}.
\end{align*}
```
Finally, we can use expressions for the derivatives of dot and cross product:
```math
\nabla_i (\kappa\mathbf{b})_j =
\frac{1}{|\mathbf{e}^{j-1}| |\mathbf{e}^{j}| + \mathbf{e}^{j-1} \cdot \mathbf{e}^{j}}
\begin{cases}
...
**c)** Implement the functions `computeBendForce` and `computeTwistForce`. Make sure to read the relevant sections in the paper carefully - in particular, note that $L$ is half the total length of the curve, and pay attention to the transposes in the force equations.

**answer:** See `elasticRods.jl`.

**d)** Fixing the material parameters $\alpha$ and $\beta$ (`bendModulus` and `twistModulus` in the code), plot the maximum displacement over time for a variety of total twist values. Try to determine the critical twist at which the Michell's instability phase transition occurs. Does it correspond to the theoretical value $\theta = 2\pi\sqrt{3}\alpha/\beta$?

**answer:** A series of experiments shows that the theoretical value $\theta = 2\pi\sqrt{3}\alpha/\beta$ only approximately matches the critical twist at which the Michell's instability phase transition occurs. In practice, the critical twist is usually slightly bigger ($1.04\theta$ - $1.09\theta$).
